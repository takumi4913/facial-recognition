...
if 0 <= alpha <= 1:
    alpha = input_alpha
# [load]
src1 = cv.imread(cv.samples.findFile('3.png'))
#src2 = cv.imread(cv.samples.findFile('4.jpg'))
# [load]
if src1 is None:
...
#------------------------------------------------------------------------------
# 画像を読み込む。
img4 = cv2.imread("neko.jpg")

# グレースケールに変換する。
gray1 = cv2.cvtColor(img4, cv2.COLOR_BGR2GRAY)

# 2値化する。
thresh, binary = cv2.threshold(gray1, 230, 255, cv2.THRESH_BINARY_INV)
    ...
    for (x, y, w, h) in faces:
        ...
        if maxindex == 0:
            ...
            for (ex, ey, ew, eh) in eye:
                ...
                img1 = rgba
                dst = cv.addWeighted(src1, alpha, frame, beta, 0.0)
                gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                frame=image(rgba,((ex+x)-20,(ey+y)+30,(ex+ew+x)-20,(ey+eh+y)+30))
                #ret, img = cap.read()
                #gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
